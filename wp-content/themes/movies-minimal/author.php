...
<main class="mx-auto max-w-[1000px] px-6 py-16 md:px-8 md:py-24">
  ...
  <header class="mt-[48px] grid gap-10 md:grid-cols-[minmax(0,1fr)_280px] md:items-start">
    <div>
      ...
      <?php if ($author_category_stats['categories'] !== []) : ?>
        <div class="mt-6">
          <p class="theme-muted text-xs font-bold uppercase tracking-[0.18em]">Category Mix</p>
          <ul class="mt-3 space-y-3">
            <?php foreach ($author_category_stats['categories'] as $category_stat) : ?>
              <li class="theme-body flex max-w-[280px] items-baseline justify-between gap-4 text-sm">
    ...
    <?php if ($author_bio !== '') : ?>
      <aside class="theme-border border-t pt-4 md:pt-0 ">
        <div>
  ...
  <?php if ($author_categories !== []) : ?>
    <nav class="mt-8 flex flex-wrap gap-3">
      <a
        class="theme-strong text-sm font-bold uppercase tracking-[0.08em] transition-opacity hover:opacity-70 <?php echo $active_category === '' ? 'underline underline-offset-4' : ''; ?>"
      ...
      <?php foreach ($author_categories as $category) : ?>
        ...
        ?>
        <a
          class="theme-strong text-sm font-bold uppercase tracking-[0.08em] transition-opacity hover:opacity-70 <?php echo $is_active ? 'underline underline-offset-4' : ''; ?>"
      ...
      <?php while (have_posts()) : the_post(); ?>
        ...
        <article>
          <a class="block no-underline transition-opacity hover:opacity-70" href="<?php the_permalink(); ?>">
            ...
            <h2 class="theme-strong mt-4 text-xl tracking-[-0.04em]">
              <?php the_title(); ?>
            </h2>
